<template>
  <div>
    ...
    ></CusBtnGroup>
    <!-- 表格组件 -->
    <CusTable ref="tableRef" :init="api.sys.role.getRoleListPage">
          ...
          <div class="flex flex-wrap items-center justify-around">
            ...
            <div class="flex-1 p-1">
              <CusBtn
                ...
                :plain="true"
                :show-tip="true"
                tip="强制删除，对于的用户会失去该角色，系统默认的三个角色无法删除"
                size="small"
                @click="delForceHan(scope.row)"
                >强制删除</CusBtn
              >
            </div>
          </div>
...
const delHan = async (row: any) => {
  const res: Res = await api.sys.role.delRole({ id: row.id });
  tt.msg.success(res.message);
  await initData();
};

// 删除事件
const delForceHan = async (row: any) => {
  const res: Res = await api.sys.role.delForceRole({ id: row.id });
  tt.msg.success(res.message);
  await initData();
};
// 初始化事件
