<template>
    <el-dialog v-model="vis" title="新增">
    ...
    const data = ref([
        <#list tableFields as item>
            <#if item.columnName != 'update_time' && item.columnName != 'update_user' && item.columnName != 'create_user' && item.columnName != 'create_time' && item.columnName!= 'id'>
                <#-- 文本框       -->
                <#if item.columnName?contains('content')  && item.dataType == 'text'>
                {
                    type: 'textarea',
                    name: '${item.javaName}',
                    default: '',
    ...
    const addHandler = async () => {
        ...
        await cusFormRef.value.formRef.validate(async (valid: any, fields: any) => {
            if (valid) {
                ...
                const result: Res = await api.${tablePrefix}.${lowerModelName}.add${modelName}({
